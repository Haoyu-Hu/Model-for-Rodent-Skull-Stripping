...
class VolumeDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if isinstance(self.rimg_files, list):
            ...
            if self.rotation:
                vector = np.random.uniform(0, 1, (1, 3))
                vec_norm = normalize(vector).ravel()
                theta = np.random.uniform(0, 15)
                rimg_sitk = sitk.ReadImage(os.path.join(self.rimg_dir, self.rimg_files[index]))
                rimg_rotated = rotation3d(rimg_sitk, theta, vec_norm).astype(np.float32)
                rimg_rotated = (rimg_rotated-rimg_rotated.min())/(rimg_rotated.max()-rimg_rotated.min())
                rimg_rotated = torch.from_numpy(rimg_rotated)
                Out.append(rimg_rotated)
            else:
        ...
        if isinstance(self.cimg_files, list):
            ...
            if self.rotation:
                ...
                vec_norm = normalize(vector).ravel()
                theta = np.random.uniform(0, 15)
                cimg_sitk = sitk.ReadImage(os.path.join(self.cimg_dir, self.cimg_files[index]))
                cimg_rotated = rotation3d(cimg_sitk, theta, vec_norm).astype(np.float32)
                cimg_rotated = (cimg_rotated-cimg_rotated.min())/(cimg_rotated.max()-cimg_rotated.min())
                cimg_rotated = torch.from_numpy(cimg_rotated)
                Out.append(cimg_rotated)
        ...
        if isinstance(self.bmsk_files, list):
            ...
            if self.rotation:
                bmsk_sitk = sitk.ReadImage(os.path.join(self.bmsk_dir, self.bmsk_files[index]))
                bmsk_rotated = np.array(rotation3d(bmsk_sitk, theta, vec_norm)>0, dtype=np.int64)
                if isinstance(self.cimg_files, list):
                    bmsk_rotated = np.resize(bmsk_rotated, cimg_rotated.shape)
                elif isinstance(self.rimg_files, list):
                    bmsk_rotated = np.resize(bmsk_rotated, rimg_rotated.shape)
                bmsk_rotated = torch.from_numpy(bmsk_rotated)
                Out.append(bmsk_rotated)
            else:
                if isinstance(self.cimg_files, list):
                    bmsk_nii = resample_img(bmsk_nii, target_affine=cimg_nii.affine, target_shape=cimg_nii.shape, interpolation='nearest')
                elif isinstance(self.rimg_files, list):
                    bmsk_nii = resample_img(bmsk_nii, target_affine=rimg_nii.affine, target_shape=rimg_nii.shape, interpolation='nearest')
...
#             bmsk = skTrans.resize(bmsk, rimg.shape, order=1, preserve_range=True)
#             print(bmsk.shape)
